...
%   output: classification of sensor nodes and command nodes, and the
%           cooresponding sensors scores.
...
nodes = variable_id;
% i is the index of ith variable in nodes.
unique_time = {};
...
for i = 1: length(nodes)
    ...
    %     if ratio > command_threshold
    %         sensorincluding_id = [sensorincluding_id; nodes(i), ratio, i, length(unique_time{i}), sum(trigger_time(:,i) ~= 0)];
    %     else
    %         commandincluding_id = [commandincluding_id; nodes(i), ratio, i, length(unique_time{i}), sum(trigger_time(:,i) ~= 0)];
    %     end
...
% save('./data/sensor_command_classify20201231.mat', 'sensorincluding_id', 'commandincluding_id')
...
% command_update_threshold = 0.123;
%
%
...
%             % to-do-list: can separate as multiple, and for cycle loop
%
...
%             end
%
%             command_delayed_set(update, :) = {sensorincluding_id(i , :), {sen_rise_delay}, sen_rise_delay_time, 1} ; % 1 -- �������ӳ�
%             update = update+1;
%
%         else
...
%
%             sensor_set = [sensor_set; command_delayed_set{1:min(update-2, 1), 1} ];
%
...
%
% % % % % % search the "command_id" set in commandincluding_id set
% % % % if the transition of variables in commandincluding_id cannot be determined by the identified sensor set
% % % % the variable itself belongs to sensor set
%
%
%
% % len_sensor_set = length(sensor_set) ;
%
% % do xxx while sss is larger than ...
%
% for i =  1: length(commandincluding_id)
%
%     command_ca = commandincluding_id(i, 3) ;
%
%
%     unique_time{command_ca} = setdiff(setdiff(trigger_time(:, command_ca ), []), setdiff(trigger_time(:, sensor_set(:,3) ), []) );
%
%     ratio = length(unique_time{command_ca})/sum(trigger_time(:, command_ca) ~= 0);
%
%     if ratio > threshold
%         A = commandincluding_id(i, :);
%         A(1, 2) = ratio;
%         sensor_set = [sensor_set; A] ;
%     else
%         A = commandincluding_id(i, :);
%         A(1, 2) = ratio;
%         command_set = [command_set; A] ;
%     end
% end
