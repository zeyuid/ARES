...
class CpuWidget(QWidget):
	...
	def __init__(self, mainWidget, parent=None):
		...
		client.haveMemoryUpdate.connect(self.__handleMemoryUpdate)  # update the CPU state in GUI?
	...
	def __handleMemoryUpdate(self, memAreas):  # update the widget of CPU
		for mdiWin in self.stateMdi.subWindowList():
			win = mdiWin.widget()
			win.setMemories(memAreas)

	def __handleGuiRunStateChange(self, newState):  # work when server connected/disconnected, download program
		client = self.getSimClient()
