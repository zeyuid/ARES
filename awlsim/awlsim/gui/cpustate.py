...
class StateWindow(QWidget):
	...
	# Send a request to the CPU to write memory. # Send a request to write the CPU memory.
...
class State_Mem(StateWindow):
	...
	def __init__(self, client, addrSpace, parent=None):
		...
		if addrSpace == AbstractDisplayWidget.ADDRSPACE_E:
			self.setWindowIcon(getIcon("inputs"))   # the I register
		elif addrSpace == AbstractDisplayWidget.ADDRSPACE_A:
			self.setWindowIcon(getIcon("outputs"))   # the Q register
		elif addrSpace == AbstractDisplayWidget.ADDRSPACE_M:
			self.setWindowIcon(getIcon("flags"))   # the M register
		elif addrSpace == AbstractDisplayWidget.ADDRSPACE_DB:
			self.setWindowIcon(getIcon("datablock"))   # the DB register
	...
	def __changed(self):
		...
		self._writeCpuMemory((memArea,))  # Send a request to write the CPU memory.
	...
	def setMemory(self, memArea):  # change the CPU memory according to COU widget
