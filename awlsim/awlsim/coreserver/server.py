...
#@cy		cdef AwlSim sim
		...
		while self.__state != self.STATE_EXIT:
			try:
				...
				if self.__state == self.STATE_RUN:  # can change CPU memory using GUI client, here.
					while self.__running: # the kth cycle
						sim.runCycle() # running the CPU with the (k-1)th CPU memory
						if self.__haveAnyMemReadReq:
							self.__handleMemReadReqs()  # send (k)th CPU memory to the GUI
						self.__handleCommunication()		#@cy-win # load the (k)th CPU memory from the GUI
