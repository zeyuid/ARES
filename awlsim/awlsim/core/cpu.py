...
#@cy	cdef uint8_t fetchOutputByte(self, uint32_t byteOffset):
	...
	# Store a range in the 'output' memory area.
	# 'byteOffset' is the byte offset into the output area.
	# 'data' is a bytearray.
	# This raises an AwlSimError, if the access if out of range.

	def storeOutputRange(self, byteOffset, data):
		dataLen = len(data)
		if byteOffset + dataLen > self.specs.nrOutputs:
			raise AwlSimError("Store to output process image region "
							  "is out of range "
							  "(imageSize=%d, storeOffset=%d, storeSize=%d)." % (
								  self.specs.nrInputs, byteOffset, dataLen))
		dataBytes = self.outputs.getRawDataBytes()
		dataBytes[byteOffset: byteOffset + dataLen] = data


	def storeOutputByte(self, byteOffset, data):
		if byteOffset >= self.specs.nrOutputs:
			raise AwlSimError("Store to output process image byte "
							  "is out of range "
							  "(imageSize=%d, storeOffset=%d)." % (
								  self.specs.nrInputs, byteOffset))
		self.outputs.getRawDataBytes()[byteOffset] = data
