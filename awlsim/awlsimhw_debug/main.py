...
class HardwareInterface_Debug(AbstractHardwareInterface): #+cdef #@nocov
	...
	def readInputs(self):
		...
		if not (self.cnt % self.cycle == 0):
			...
			self.__Process_GPIO_commandlog = self.__Process_GPIO.command_SCADA
...
#@cy	cdef ExBool_t directWriteOutput(self, uint32_t accessWidth, uint32_t accessOffset, bytearray data) except ExBool_val:
	...
	def output_detection(self, command_log):
		global CuSUM_vector

		# S = zeros(size(Command_meas, 1), 1);
		# Alarm = zeros(size(Command_meas, 1), 1);

		if not self.hw_shutdown:
			command_estimated = self.__Commands
		else:
			print('there is no attack happens')

		command_logged = self.__Process_GPIO_commandlog()   # obtain the command_looged from SCADA request

		# only store 2 estimated steps
		# compare the command_estimate with command_log, using CUSUM
		command_errors = command_estimated - command_logged
