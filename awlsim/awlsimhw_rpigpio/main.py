...
class RpiGPIO_BitMapping(object): #+cdef
	...
	def build(self):
		...
		self.currentOutputValues = [None] * 8 #@nocy
		self.size = 0
		for bitOffset, bcmNumber in sorted(dictItems(self.__bit2bcm), key=lambda x: x[0]):
...
class RpiGPIO_HwInterface(AbstractHardwareInterface): #+cdef
	...
	def doStartup(self):
		...
		"""
		# Get the configuration, a list of (name, value) tuples, Zeyu
	...
	def __mapGPIO(self, configs, nameRegEx, gpioDir, byteBaseOffset):
		...
		for address, bcmNumber in configs:
			...
			mapping = mapDict.setdefault(byteBaseOffset + byteOffset, RpiGPIO_BitMapping())
			mapping.setBit(bitOffset, bcmNumber)
			try:
				if gpioDir == RPi_GPIO.IN:
					RPi_GPIO.setup(bcmNumber,  gpioDir, pull_up_down = RPi_GPIO.PUD_DOWN)
				else:
					RPi_GPIO.setup(bcmNumber, gpioDir,  initial = RPi_GPIO.LOW)
			except RuntimeError as e: #@nocov
				self.raiseException("Failed to init Raspberry Pi "
		...
		byteOffsetList = []
		bitMappingList = []
		for byteOffset, bitMapping in sorted(dictItems(mapDict), key=lambda x: x[0]):
